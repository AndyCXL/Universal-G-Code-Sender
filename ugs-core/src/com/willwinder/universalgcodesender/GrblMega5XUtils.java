...
public class GrblMega5XUtils {
    ...
    static protected Position parseProbePosition(final String response, final Units units, String axisOrder) {
     ...
     * @param axisOrder string the XYZABC axis order read from the machine
    ...
    static protected ControllerStatus getStatusFromStatusString(
        ...
        if (!version.hasCapability(GrblCapabilitiesConstants.V1_FORMAT)) {
            return getStatusFromStatusStringLegacy(status, version, reportingUnits, axisOrder);
        } else {
     ...
     * @return the parsed controller status
     */
    private static ControllerStatus getStatusFromStatusStringLegacy(String status, Capabilities version, Units reportingUnits, String axisOrder) {
        ...
        return new ControllerStatus(
                state,
                getMachinePositionFromStatusString(status, version, reportingUnits, axisOrder),
                getWorkPositionFromStatusString(status, version, reportingUnits, axisOrder));
    }
