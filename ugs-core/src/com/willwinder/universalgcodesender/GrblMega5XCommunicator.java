 ...
 * @author wwinder
 */
public class GrblMega5XCommunicator extends BufferedCommunicator {
    ...
    protected GrblMega5XCommunicator() {}

    /**
     * This constructor is for dependency injection so a mock serial device can
     * act as GRBL.
     */
    protected GrblMega5XCommunicator(LinkedBlockingDeque<GcodeCommand> cb, LinkedBlockingDeque<GcodeCommand> asl, Connection c) {
        super(cb, asl);
        this.connection = c;
    ...
    public int getBufferSize() {
        return GrblMega5XUtils.GRBL_RX_BUFFER_SIZE;
    }

    @Override
    protected boolean processedCommand(String response) {
        return GrblMega5XUtils.isOkErrorAlarmResponse(response);
    }
